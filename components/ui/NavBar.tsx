...
const isCurrent = (current: string, link: Link) => {
...
function NavBar() {
  ...
  return (
    <div className="bg-white shadow fixed top-0 left-0 right-0">
          ...
          <div className="hidden sm:flex sm:items-center">
            {links.map((link) => (
              <Link key={link.url} href={link.url}>
                <a
                  className={
                    classnames.all + (isCurrent(current, link) ? classnames.current : classnames.other)
                  }
                >
                  {link.label}
                </a>
            ...
            <div className="flex flex-col">
              ...
              {links.map((link) => (
                <Link key={link.url} href={link.url}>
                  <a
                    className={
                      mobileClassnames.all + (isCurrent(current, link) ? mobileClassnames.current : mobileClassnames.other)
                    }
                  >
                    {link.label}
                  </a>
